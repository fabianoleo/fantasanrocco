...
function leaderboardRows() {
  ...
  cache = db.prepare(`
    SELECT u.id, u.nickname, u.avatar_path,
           ...
           -- I punti fatti con le MISSIONI, da soli. Sono il primo spareggio:
           -- vedi l'ORDER BY qui sotto.
           COALESCE(SUM(CASE WHEN s.status='approved' THEN m.points ELSE 0 END), 0) AS puntiMissioni,
           -- Secondo spareggio: i due record dei mini-giochi sommati. Sono
           -- grandezze diverse (punti del Runner e metri del Jetpack) e
           -- sommarle non ha un senso fisico: serve solo a mettere in fila due
           -- persone che hanno gli stessi punti, e premia chi ha giocato di piu'.
    ...
    WHERE u.role = 'user'
    GROUP BY u.id
    -- A PARI PUNTI passa avanti chi ne ha fatti di piu' con le MISSIONI.
    -- I punti sono tutti uguali una volta in classifica, ma non si prendono
    -- allo stesso prezzo: una missione vuol dire uscire di casa, trovare la
    -- persona giusta, scattare e farsi approvare la prova; la ruota e la slot
    -- si girano dal divano. A parita' di totale premia chi ha fatto la strada
    -- piu' lunga.
    -- Poi vengono i record dei mini-giochi, e per ultimo l'ordine d'iscrizione,
    -- che e' l'unico criterio che non finisce mai in parita'.
    ORDER BY points DESC, puntiMissioni DESC, spareggio DESC, u.created_at ASC
  `).all();
